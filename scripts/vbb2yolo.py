...
def convert_annotation(sname,anns,img):
    w = img['width']
    h = img['height']
    out_file = open('data/scut/labels/%s/%s.txt'%(sname,img['file_name'].split('.')[0]),'w')
...
wd = getcwd()
for s in sets:
    if not os.path.exists('data/scut/labels/%s'%(s)):
        os.makedirs('data/scut/labels/%s'%(s))
    list_file = open('data/scut/%s.txt'%(s),'w')
...
if not os.path.exists('data/scut/images'):
    pdt.scut.seqs2imgs('scut_train','%s/data/scut/videos'%(wd),'%s/data/images/train'%(wd),1)
    pdt.scut.seqs2imgs('scut_test','%s/data/scut/videos'%(wd),'%s/data/images/test'%(wd),1)
else:
    print "There is images directory.If you need extract images, please del the images dir."